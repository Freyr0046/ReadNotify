...
class MainViewModelTest {
    ...
    @Test
    fun `initial state is InitChecking`() =
    ...
    @Test
    fun `permission denied transitions to PermissionDenied`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `permission granted and tts ready transitions to IdleConfig`() =
        runTest(testDispatcher) {
            every { checkNotificationAccessUseCase() } returns true
            coEvery { initializeTtsEngineUseCase() } returns Result.success(Unit)
            coEvery { getInstalledAppsUseCase() } returns
                Result.success(
    ...
    @Test
    fun `tts engine not installed transitions to EngineError`() =
        runTest(testDispatcher) {
            every { checkNotificationAccessUseCase() } returns true
            coEvery { initializeTtsEngineUseCase() } returns Result.failure(IllegalStateException("no engine"))
            every { observeEngineStateUseCase() } returns
                MutableStateFlow(
    ...
    @Test
    fun `OnScreenResumed with revoked permission force-switches to PermissionDenied`() =
        runTest(testDispatcher) {
            every { checkNotificationAccessUseCase() } returns true
            coEvery { initializeTtsEngineUseCase() } returns Result.success(Unit)
    ...
    @Test
    fun `OnAppWhitelistToggled invokes SetAppWhitelistedUseCase and updates isChecked`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `playback Speaking state transitions to TtsPlaying`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `playback Idle state returns to IdleConfig`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `OnSendTestNotificationClicked invokes SendTestNotificationUseCase exactly once`() =
        runTest(testDispatcher) {
            every { checkNotificationAccessUseCase() } returns true
            coEvery { initializeTtsEngineUseCase() } returns Result.success(Unit)
            coEvery { getInstalledAppsUseCase() } returns Result.success(emptyList())
    ...
    @Test
    fun `OnRetryEngineInitClicked recovers from EngineError to IdleConfig`() =
        runTest(testDispatcher) {
            every { checkNotificationAccessUseCase() } returns true
            coEvery { initializeTtsEngineUseCase() } returns Result.failure(IllegalStateException("no engine"))
            every { observeEngineStateUseCase() } returns
