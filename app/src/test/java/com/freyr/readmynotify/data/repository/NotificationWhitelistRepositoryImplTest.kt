...
class NotificationWhitelistRepositoryImplTest {
    ...
    @Test
    fun `observeWhitelist emits empty set by default`() =
        runTest {
    ...
    @Test
    fun `setAppEnabled true adds package to whitelist`() =
        runTest {
            val repository = createRepository()
    ...
    @Test
    fun `setAppEnabled false removes package from whitelist`() =
        runTest {
            val repository = createRepository()
            repository.setAppEnabled("com.line", true)
    ...
    @Test
    fun `whitelist persists across repository instances backed by same file`() =
        runTest {
            val dataStore =
                PreferenceDataStoreFactory.create(
