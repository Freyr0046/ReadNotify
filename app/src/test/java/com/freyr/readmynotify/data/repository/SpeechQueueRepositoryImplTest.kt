...
class SpeechQueueRepositoryImplTest {
    ...
    @Test
    fun `enqueue beyond capacity of 5 drops the newest announcement`() = runTest(UnconfinedTestDispatcher()) {
    ...
    @Test
    fun `playbackState returns to Idle after the queue drains`() = runTest(UnconfinedTestDispatcher()) {
        val ttsEngineRepository = mockk<TtsEngineRepository>()
    ...
    @Test
    fun `playbackState reflects Speaking while an announcement is in flight`() = runTest(UnconfinedTestDispatcher()) {
        val ttsEngineRepository = mockk<TtsEngineRepository>()
